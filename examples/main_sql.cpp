...
vector<long double> simulator(
  vector<long double> args, const unsigned long int rng_seed,
  const ABC::MPI_par* m
) {
    gsl_rng_set(RNG, rng_seed);
    int par1 = static_cast<int>(args[0]);  // number of dice
    int par2 = static_cast<int>(args[1]);  // number of sides on dice
...
void usage() {
    ...
    cerr << "\t\t./abc_sql abc_config_sql.json --simulate " <<
            "-n <number of simulations per database write>\n\n";
    cerr << "\t\t./abc_sql abc_config_sql.json --process --simulate " <<
            "-n <number of simulations per database write>\n\n";
}
